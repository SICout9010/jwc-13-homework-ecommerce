"use client";
import React from "react";
import Image from "next/image";

import { DM_Sans } from "next/font/google";
import { IoCartOutline } from "react-icons/io5";
import { IoPersonCircleOutline } from "react-icons/io5";
import { IoSearchOutline } from "react-icons/io5";
import Promotion from "@/components/Promotion";

import imgnav1 from "/public/images/Frame 34.png";
import imgnav2 from "/public/images/Frame 35.png";
import imgnav3 from "/public/images/Frame 36.png";

// Font section
const dm_sans = DM_Sans({
  subsets: ["latin"],
  weight: ["400", "700"],
});

function Header() {
  const [showMoreNev, setShowMoreNev] = React.useState(false);

  return (
    <nav>
      <Promotion />
      <div className="w-full flex flex-col items-center bg-white absolute z-50">
        <div className="flex items-center justify-between min-w-[1240px] min-h-[105px]">
          <h1 className={`${dm_sans.className} font-bold text-3xl`}>JWC.CO</h1>
          <ul className="flex justify-between gap-4 text-base font-normal text-black">
            <li
              onMouseOver={() => setShowMoreNev(!showMoreNev)}
              onMouseOut={() => setShowMoreNev(false)}
              className="flex gap-1 items-center cursor-pointer"
            >
              Shop
              {showMoreNev ? (
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="pointer-events-none"
                >
                  <path
                    d="M13.5306 10.468L8.53063 5.46804C8.46095 5.39812 8.37816 5.34265 8.28699 5.30479C8.19583 5.26694 8.09809 5.24745 7.99938 5.24745C7.90067 5.24745 7.80293 5.26694 7.71176 5.30479C7.6206 5.34265 7.53781 5.39812 7.46813 5.46804L2.46813 10.468C2.32723 10.6089 2.24808 10.8 2.24808 10.9993C2.24808 11.1986 2.32723 11.3896 2.46813 11.5305C2.60902 11.6714 2.80012 11.7506 2.99938 11.7506C3.19864 11.7506 3.38973 11.6714 3.53063 11.5305L8 7.06117L12.4694 11.5312C12.6103 11.6721 12.8014 11.7512 13.0006 11.7512C13.1999 11.7512 13.391 11.6721 13.5319 11.5312C13.6728 11.3903 13.7519 11.1992 13.7519 10.9999C13.7519 10.8007 13.6728 10.6096 13.5319 10.4687L13.5306 10.468Z"
                    fill="black"
                  />
                </svg>
              ) : (
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                  className="rotate-180 pointer-events-none"
                >
                  <path
                    d="M13.5306 10.468L8.53063 5.46804C8.46095 5.39812 8.37816 5.34265 8.28699 5.30479C8.19583 5.26694 8.09809 5.24745 7.99938 5.24745C7.90067 5.24745 7.80293 5.26694 7.71176 5.30479C7.6206 5.34265 7.53781 5.39812 7.46813 5.46804L2.46813 10.468C2.32723 10.6089 2.24808 10.8 2.24808 10.9993C2.24808 11.1986 2.32723 11.3896 2.46813 11.5305C2.60902 11.6714 2.80012 11.7506 2.99938 11.7506C3.19864 11.7506 3.38973 11.6714 3.53063 11.5305L8 7.06117L12.4694 11.5312C12.6103 11.6721 12.8014 11.7512 13.0006 11.7512C13.1999 11.7512 13.391 11.6721 13.5319 11.5312C13.6728 11.3903 13.7519 11.1992 13.7519 10.9999C13.7519 10.8007 13.6728 10.6096 13.5319 10.4687L13.5306 10.468Z"
                    fill="black"
                  />
                </svg>
              )}
            </li>
            <li>On Sale</li>
            <li>New Arrivals</li>
            <li>Brands</li>
          </ul>
          {/* Search bar */}
          <div className="min-w-[590px] flex bg-[#F0F0F0] p-3 rounded-[62px] gap-4">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M21.7959 20.2041L17.3437 15.75C18.6787 14.0104 19.3019 11.8282 19.087 9.64606C18.8722 7.46389 17.8353 5.44515 16.1867 3.99936C14.5382 2.55356 12.4014 1.78898 10.2098 1.8607C8.01829 1.93243 5.93607 2.83509 4.38558 4.38558C2.83509 5.93607 1.93243 8.01829 1.8607 10.2098C1.78898 12.4014 2.55356 14.5382 3.99936 16.1867C5.44515 17.8353 7.46389 18.8722 9.64606 19.087C11.8282 19.3019 14.0104 18.6787 15.75 17.3437L20.2059 21.8006C20.3106 21.9053 20.4348 21.9883 20.5715 22.0449C20.7083 22.1015 20.8548 22.1307 21.0028 22.1307C21.1508 22.1307 21.2973 22.1015 21.4341 22.0449C21.5708 21.9883 21.695 21.9053 21.7997 21.8006C21.9043 21.696 21.9873 21.5717 22.044 21.435C22.1006 21.2983 22.1298 21.1517 22.1298 21.0037C22.1298 20.8557 22.1006 20.7092 22.044 20.5725C21.9873 20.4357 21.9043 20.3115 21.7997 20.2069L21.7959 20.2041ZM4.12499 10.5C4.12499 9.23914 4.49888 8.0066 5.19938 6.95823C5.89987 5.90987 6.89551 5.09277 8.06039 4.61026C9.22527 4.12775 10.5071 4.00151 11.7437 4.24749C12.9803 4.49347 14.1162 5.10063 15.0078 5.99219C15.8994 6.88375 16.5065 8.01966 16.7525 9.25629C16.9985 10.4929 16.8722 11.7747 16.3897 12.9396C15.9072 14.1045 15.0901 15.1001 14.0418 15.8006C12.9934 16.5011 11.7608 16.875 10.5 16.875C8.80977 16.8733 7.18927 16.201 5.99411 15.0059C4.79894 13.8107 4.12673 12.1902 4.12499 10.5Z"
                fill="black"
                fill-opacity="0.4"
              />
            </svg>

            <input
              type="text"
              placeholder="Search for products..."
              className="w-full bg-[#F0F0F0] outline-none"
            />
          </div>
          <div className="flex">
            <svg
              width="62"
              height="24"
              viewBox="0 0 62 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M9.375 20.25C9.375 20.6208 9.26503 20.9834 9.059 21.2917C8.85298 21.6 8.56014 21.8404 8.21753 21.9823C7.87492 22.1242 7.49792 22.1613 7.1342 22.089C6.77049 22.0166 6.4364 21.838 6.17417 21.5758C5.91195 21.3136 5.73337 20.9795 5.66103 20.6158C5.58868 20.2521 5.62581 19.8751 5.76773 19.5325C5.90964 19.1899 6.14996 18.897 6.45831 18.691C6.76665 18.485 7.12916 18.375 7.5 18.375C7.99728 18.375 8.47419 18.5725 8.82582 18.9242C9.17745 19.2758 9.375 19.7527 9.375 20.25ZM17.25 18.375C16.8792 18.375 16.5166 18.485 16.2083 18.691C15.9 18.897 15.6596 19.1899 15.5177 19.5325C15.3758 19.8751 15.3387 20.2521 15.411 20.6158C15.4834 20.9795 15.662 21.3136 15.9242 21.5758C16.1864 21.838 16.5205 22.0166 16.8842 22.089C17.2479 22.1613 17.6249 22.1242 17.9675 21.9823C18.3101 21.8404 18.603 21.6 18.809 21.2917C19.015 20.9834 19.125 20.6208 19.125 20.25C19.125 19.7527 18.9275 19.2758 18.5758 18.9242C18.2242 18.5725 17.7473 18.375 17.25 18.375ZM22.0753 7.08094L19.5169 15.3966C19.3535 15.9343 19.0211 16.4051 18.569 16.739C18.1169 17.0729 17.5692 17.2521 17.0072 17.25H7.77469C7.2046 17.2482 6.65046 17.0616 6.1953 16.7183C5.74015 16.3751 5.40848 15.8936 5.25 15.3459L2.04469 4.125H1.125C0.826631 4.125 0.540483 4.00647 0.329505 3.7955C0.118526 3.58452 0 3.29837 0 3C0 2.70163 0.118526 2.41548 0.329505 2.2045C0.540483 1.99353 0.826631 1.875 1.125 1.875H2.32687C2.73407 1.87626 3.12988 2.00951 3.45493 2.25478C3.77998 2.50004 4.01674 2.84409 4.12969 3.23531L4.81312 5.625H21C21.1761 5.62499 21.3497 5.6663 21.5069 5.74561C21.664 5.82492 21.8004 5.94001 21.905 6.08164C22.0096 6.22326 22.0795 6.38746 22.1091 6.56102C22.1387 6.73458 22.1271 6.91266 22.0753 7.08094ZM19.4766 7.875H5.45531L7.41375 14.7281C7.43617 14.8065 7.48354 14.8755 7.54867 14.9245C7.6138 14.9736 7.69315 15.0001 7.77469 15H17.0072C17.0875 15.0002 17.1656 14.9746 17.2303 14.927C17.2949 14.8794 17.3426 14.8123 17.3662 14.7356L19.4766 7.875Z"
                fill="black"
              />
              <path
                d="M50 1.875C47.9975 1.875 46.0399 2.46882 44.3749 3.58137C42.7098 4.69392 41.4121 6.27523 40.6457 8.12533C39.8794 9.97543 39.6789 12.0112 40.0696 13.9753C40.4602 15.9393 41.4245 17.7435 42.8405 19.1595C44.2566 20.5755 46.0607 21.5398 48.0247 21.9305C49.9888 22.3211 52.0246 22.1206 53.8747 21.3543C55.7248 20.5879 57.3061 19.2902 58.4186 17.6251C59.5312 15.9601 60.125 14.0025 60.125 12C60.122 9.3156 59.0543 6.74199 57.1562 4.84383C55.258 2.94567 52.6844 1.87798 50 1.875ZM45.4597 18.4284C45.9819 17.7143 46.6653 17.1335 47.4542 16.7331C48.2431 16.3327 49.1153 16.124 50 16.124C50.8847 16.124 51.7569 16.3327 52.5458 16.7331C53.3347 17.1335 54.0181 17.7143 54.5403 18.4284C53.2134 19.3695 51.6268 19.875 50 19.875C48.3732 19.875 46.7867 19.3695 45.4597 18.4284ZM47.375 11.25C47.375 10.7308 47.529 10.2233 47.8174 9.79163C48.1058 9.35995 48.5158 9.0235 48.9955 8.82482C49.4751 8.62614 50.0029 8.57415 50.5121 8.67544C51.0213 8.77672 51.489 9.02673 51.8562 9.39384C52.2233 9.76096 52.4733 10.2287 52.5746 10.7379C52.6759 11.2471 52.6239 11.7749 52.4252 12.2545C52.2265 12.7342 51.8901 13.1442 51.4584 13.4326C51.0267 13.721 50.5192 13.875 50 13.875C49.3038 13.875 48.6361 13.5984 48.1438 13.1062C47.6516 12.6139 47.375 11.9462 47.375 11.25ZM56.1875 16.8694C55.4583 15.9419 54.5289 15.1914 53.4688 14.6737C54.1444 13.9896 54.6026 13.1208 54.7858 12.1769C54.9689 11.2329 54.8688 10.2558 54.498 9.36861C54.1273 8.4814 53.5024 7.72364 52.702 7.19068C51.9017 6.65771 50.9616 6.37334 50 6.37334C49.0384 6.37334 48.0983 6.65771 47.298 7.19068C46.4976 7.72364 45.8727 8.4814 45.502 9.36861C45.1312 10.2558 45.0311 11.2329 45.2142 12.1769C45.3974 13.1208 45.8556 13.9896 46.5313 14.6737C45.4711 15.1914 44.5417 15.9419 43.8125 16.8694C42.8966 15.7083 42.3261 14.3129 42.1664 12.8427C42.0067 11.3725 42.2641 9.88711 42.9093 8.55644C43.5544 7.22578 44.5612 6.10366 45.8144 5.31855C47.0676 4.53343 48.5165 4.11703 49.9953 4.11703C51.4741 4.11703 52.9231 4.53343 54.1762 5.31855C55.4294 6.10366 56.4362 7.22578 57.0814 8.55644C57.7265 9.88711 57.984 11.3725 57.8242 12.8427C57.6645 14.3129 57.094 15.7083 56.1781 16.8694H56.1875Z"
                fill="black"
              />
            </svg>
          </div>
        </div>

        {showMoreNev && (
          <div className="w-full flex gap-1 mx-[100px] max-w-[1240px] min-h-[250px]">
            <div className="flex">
              <div className="flex gap-[79px] font-normal text-black">
                <div className="flex flex-col gap-6 w-[101px]">
                  <h1 className="font-extrabold relative">Shop Branch</h1>
                  <p>Bangkok</p>
                  <p>Tokyo</p>
                  <p>New York</p>
                  <p>London</p>
                </div>
                <div className="flex flex-col gap-6 w-[101px]">
                  <br />
                  <p>Bangkok</p>
                  <p>Tokyo</p>
                  <p>New York</p>
                  <p>London</p>
                </div>
                <div className="flex flex-col gap-6 w-[101px]">
                  <br />
                  <p>Bangkok</p>
                  <p>Tokyo</p>
                  <p>New York</p>
                  <p>London</p>
                </div>
              </div>

              <div className="ml-[60px] flex items-center justify-between gap-10">
                <Image src={imgnav1} alt="imgnav" />
                <Image src={imgnav2} alt="imgnav" />
                <Image src={imgnav3} alt="imgnav" />
              </div>
            </div>
          </div>
        )}
      </div>
    </nav>
  );
}

export default Header;
