...
function PWcustomer() {
  ...
  return (
    <main className="flex flex-col items-center mb-[180px] w-full overflow-hidden relative">
      ...
      >
        {com_customer.comment_customer.map((comment) => (
          <SwiperSlide key={comment.id} className="blur-md invert filter-none">
            <div
              key={comment.id}
              className={`relative right-[40%] flex flex-col min-w-[400px] min-h-[240px] rounded-[20px] border border-[#0000001A] p-6 transition-all duration-300
                `}
            >
              <Rating5star />
