...
function PWcustomer() {
  ...
  // Function to navigate to the next comment
  const nextComment = () => {
    setCenteredIndex((prevIndex) =>
      prevIndex < com_customer.comment_customer.length - 1
        ? prevIndex + 1
        : prevIndex
    );
  };

  // Function to navigate to the previous comment
  const prevComment = () => {
    setCenteredIndex((prevIndex) =>
  ...
  return (
    <main className="flex flex-col items-center mb-[180px] w-full overflow-hidden relative">
      <div className="carousel flex flex-col w-[1240px]">
        <h1 className="text-6xl font-bold mb-4">OUR HAPPY CUSTOMERS</h1>

        <div className="flex justify-end items-end mb-8">
          <div onClick={nextComment} className="flex items-center gap-4">
            <svg
              width="24"
              height="24"
              viewBox="0 0 24 24"
      ...
      <div className="flex gap-6 font-normal">
        ...
        {com_customer.comment_customer.map((comment, index) => (
          <div
            key={comment.id}
            id={`slide${comment.id}`}
            className={`carousel-item flex flex-col min-w-[400px] min-h-[240px] rounded-[20px] border-2 border-[#0000001A] p-6
              ${index === centeredIndex ? '' : 'blur-sm'}
              `}
          >
            <Rating5star />
